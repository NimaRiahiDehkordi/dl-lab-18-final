...
class FCN_SS(object):
    ...
    def train(self, opt, CropAndPad_percentage,Flip_percentage,Scale_percentage,Rotate_degree):
        TotalLOSS = 0.0
        self.opt = opt
        self.batch_size = opt.batch_size
        self.counter = 0
        ...
        with sv.managed_session() as sess:
            ...
            train_label_aug = seq_deterministic.augment_images(np.reshape(train_label, [-1, train_data.shape[1], train_data.shape[2], 1]))
            #cv2.imshow('image', train_label_aug[0])
            #cv2.waitKey(0)
            #cv2.destroyAllWindows()
            train_label_aug = np.reshape(train_label_aug, [-1, train_data.shape[1]*train_data.shape[2], 1])
