...
flags.DEFINE_float("learning_rate", 0.0004, "Learning rate for adam")
...
flags.DEFINE_integer("max_steps", 40000, "Maximum number of training iterations")
...
def main(_):
    ...
    GaussianBlurSigma = np.random.uniform(low=0.5, high=3.0)
    AdditiveGaussianNoiseScale = np.random.uniform(low=0, high=0.1)
    SaltAndPepperPercentage = np.random.uniform(low=0.2, high=0.5)
    ...
    text_file.write('GaussianBlurSigma: %f \nAdditiveGaussianNoiseScale: %f \nSaltAndPepperPercentage: %f' %(GaussianBlurSigma,AdditiveGaussianNoiseScale,SaltAndPepperPercentage))
    text_file.close()
    # Call Train
    FCN.train(FLAGS, GaussianBlurSigma, AdditiveGaussianNoiseScale, SaltAndPepperPercentage)
