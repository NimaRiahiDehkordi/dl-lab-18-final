...
class FCN_SS(object):
    ...
    def build_train_graph(self):
        ...
        self.train_label_batch = tf.placeholder(
            tf.float32, [None, opt.img_height * opt.img_width, opt.numberClasses])
        ...
        with tf.name_scope("Output_Metrics"):
            ...
            Smask = tf.placeholder(
                tf.float32, [None, opt.img_height * opt.img_width])
            Smask = tf.reshape(
                segmentationMask, (self.batch_size, opt.img_height * opt.img_width))
        ...
        with tf.name_scope("compute_loss"):
            ...
            Reshaped_labels = tf.reshape(
                self.train_label_batch, (-1, self.N_classes))

            if opt.version_net == 'FCN_Seg':
                TotalLoss = loss(
                    segMap, Reshaped_labels, self.N_classes, class_weight)
        ...
        with tf.name_scope("Training"):
            ...
            lr = tf.train.exponential_decay(
                opt.learning_rate, global_step,	(self.total_steps / 2), Decay, staircase=True)
    ...
    def Load_TrainDataset(self):
        ...
        train_data = train_data.reshape(
            (train_data.shape[0], opt.img_height, opt.img_width, 3))
        print(train_data.shape)

        train_label_path = opt.dataset_dir + \
            'Train_label_' + opt.dataset + '.npy'
        train_label = np.load(train_label_path)
        train_label = train_label.reshape(
            (train_data.shape[0], opt.img_height * opt.img_width, 1))
    ...
    def Create_batches(self, train_data, train_label):
        ...
        train_image_batch = np.zeros(
            (self.batch_size, self.img_height, self.img_width, 3), dtype=np.float32)
        # train_label_batch=np.zeros((self.batch_size,
        # self.img_height*self.img_width, self.N_classes), dtype=np.float32)
        train_label_batch = np.zeros(
            (self.batch_size, self.img_height * self.img_width, self.N_classes), dtype=np.float32)
        ...
        for i in range(self.batch_size):
            ...
            if (self.random == False):
                train_image_batch[i, :, :, :] = train_data[
                    self.counter + i, :, :, :]
                # Test Sample
                train_label_batch[i, :, :] = self.unfould(
                    train_label[self.counter + i, :, :])
            else:
                # print("RANDOM TRAINING :)")
        ...
        # train_label_batch = tf.reshape(train_label_batch, (-1,
    ...
    def unfould(self, image):
        ...
        label_training = np.zeros(
            [1,  self.height, self.width, self.N_classes])
        # for batch in range(0, self.batch_size):
        for row in range(0, self.height):
            for column in range(0, self.width):
                # print("label", int(image[0,row,column,0]))
                # if(int(image[0,row,column,0]) >= 0): # and
                # (int(image[0,row,column,0]) != 11):
                label_training[
                    0, row, column, int(image[0, row, column, 0])] = 1
                # else:
                # 	label_training[0,row,column,int(image[0,row,column,0])] = 1
        label_training = np.reshape(
            label_training, (1, self.height * self.width, self.N_classes))
        # print(label_training.shape)
        return label_training

    def train(self, opt, GaussianBlurSigma, AdditiveGaussianNoiseScale, SaltAndPepperPercentage):
        TotalLOSS = 0.0
        self.opt = opt
        self.batch_size = opt.batch_size
        self.counter = 0
        ...
        self.saver = tf.train.Saver(
            [var for var in tf.trainable_variables()], max_to_keep=100)
        self.saver2 = tf.train.Saver(
            [var for var in tf.trainable_variables()] + [self.global_step], max_to_keep=100)

        sv = tf.train.Supervisor(
            logdir=opt.logs_path, save_summaries_secs=0, saver=None)
        print("passed")
        ...
        with sv.managed_session() as sess:
            ...
            if opt.load_Model:
                ...
                # global_step_zero = tf.get_variable('global_step2', [],
            ...
            #cv2.imshow('image', train_label.reshape([-1, train_data.shape[1], train_data.shape[2], 1])[0])
            #cv2.waitKey(0)
            #cv2.destroyAllWindows()
            '''seq = iaa.Sequential(
                [
                    # apply the following augmenters to most images
                    # Pad each side by 0 to 20 percent. This adds new pixels to the sides. 
                    # These pixels will either be filled with a constant value (mode=constant) or filled with the value on the closest edge (mode=edge). 
                    # If a constant value is used, it will be a random value between 0 and 128 (sampled per image).
                    iaa.CropAndPad(
                        percent=(0, CropAndPad_percentage),
                        pad_mode=["constant"],
                        pad_cval=(0, 128)
                    ),
                    #Flip/mirror input images horizontally.
                    iaa.Fliplr(Flip_percentage),# horizontally flip 50% of all images
                    #Flip/mirror input images vertically.
                    iaa.Flipud(Flip_percentage), # vertically flip 20% of all images
                    #Scale each image to something between 50 and 100% of its original size
                    #iaa.size.Scale((Scale_percentage, Scale_percentage + 0.25)),
                    #Rotate images by -45 to 45 degrees
                    iaa.Affine(scale=(Scale_percentage, Scale_percentage + 0.25)),
                    iaa.Affine(rotate=(-Rotate_degree, Rotate_degree))
                ]
            )'''
            ...
            seq = iaa.Sequential(
                [
                    # apply the following augmenters to most images
                    iaa.GaussianBlur(sigma=(0, GaussianBlurSigma)), # blur images with a sigma of 0 to 3.0
                    iaa.AdditiveGaussianNoise(scale=AdditiveGaussianNoiseScale*255),
                    iaa.SaltAndPepper(SaltAndPepperPercentage)
                ]
            )
            seq_deterministic = seq.to_deterministic()
            train_data_aug = seq_deterministic.augment_images(train_data)
            train_label_aug = train_label
            #cv2.imshow('image', train_label_aug[0])
            #cv2.waitKey(0)
            #cv2.destroyAllWindows()
            #train_label_aug = np.reshape(train_label, [-1, train_data.shape[1]*train_data.shape[2], 1])
            #cv2.imshow('image', train_data_aug[0])
            #cv2.waitKey(0)
            #cv2.destroyAllWindows()
            train_data = np.concatenate((train_data, train_data_aug), axis=0)
            train_label = np.concatenate((train_label, train_label_aug), axis=0)
            train_data, _, train_label, _ = train_test_split(train_data,train_label, test_size=0.0)
            print(
                "=================== Starting Iterations =========================")
            global_start_time = time.time()
            for step in range(1, opt.max_steps):
                start_time = time.time()
                fetches = {"Training": self.Training, "global_step":
                           self.global_step, "incr_global_step": self.incr_global_step}

                # train_image_batch, train_label_batch =
                # self.Create_batches(train_data, train_label)
                train_image_batch, train_label_batch = self.Create_batches(
                    train_data, train_label)
                # train_image_batch_ = np.reshape(train_image_batch, (opt.batch_size, opt.img_height*opt.img_width*3))
                # train_label_batch_ = np.reshape(train_label_batch,
                # (opt.batch_size, opt.img_height*opt.img_width*12))
                ...
                # print(train_label_batch.shape)
                                # dataset =
                                # tf.data.Dataset.from_tensor_slices((train_image_batch,
                                # train_label_batch))

                results = sess.run(fetches, feed_dict={
                                   self.train_image_batch: train_image_batch, self.train_label_batch: train_label_batch})
                ...
                if step % opt.summary_freq == 0:
                    ...
                    print("Epoch: [%2d] [%5d/%5d] time: %4.4f/it loss: %3f" %
                          (train_epoch, train_step, opt.steps_per_epoch, time.time() - start_time, results["loss"]))
    ...
    def build_test_graph(self):
        input_img = tf.placeholder(
            tf.float32, [self.batch_size, self.img_height, self.img_width, 3], name='input_image')
    ...
    def loadTest_set(self, opt):
        ...
        test_data = test_data.reshape(
            (test_data.shape[0], self.height, self.width, 3))
        print(test_data.shape)

        test_label_path = opt.dataset_dir + \
            'Test_label_' + opt.dataset + '.npy'
        print(test_label_path)
        test_label = np.load(test_label_path)
        test_label = test_label.reshape(
            (test_label.shape[0], self.height * self.width, 1))

        if(self.Test == True):
            test_label = np.zeros(
                (test_data.shape[0], self.height * self.width, 1))

        # shape is (233, 50176, 12)
        print(test_label.shape)
        test_label_batch = np.zeros(
            (test_label.shape[0], self.height * self.width, self.N_classes), dtype=np.float32)
    ...
    def loadValidation_set(self, opt):
        ...
        # test_label_path = opt.dataset_dir + 'Test_label_' + 'Camvid2' +
        # '.npy'
        test_label_path = opt.dataset_dir + \
            'Test_label_' + opt.dataset + '.npy'
